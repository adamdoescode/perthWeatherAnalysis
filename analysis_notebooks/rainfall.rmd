...
#group by no rain grouping and take mean, maybe filter n>=3?
#not a great plot really! Obvs going to be confounded by time of year here. Messy.
NOAA_data %>% 
    group_by(NoRainStreakID) %>% 
    summarise(
        meanTMAX = mean(TMAX, na.rm = T),
        noRainDays = max(daysNoRain),
        n = n(),
    ) %>% drop_na() %>% 
    filter(n >= 5) %>% 
    ggplot(aes(noRainDays, meanTMAX)) +
    geom_point() +
    dark_theme_bw()
